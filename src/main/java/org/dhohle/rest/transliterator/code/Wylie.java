...
/**
 * @author http://www.digitaltibetan.org/
...
public class Wylie {
	...
	private static HashMap<String, String> m_consonant, m_subjoined, m_vowel, m_final_uni, m_final_class, m_other, m_ambiguous_wylie, m_tib_vowel_long, m_tib_caret;
	private static HashMap<Character, String> m_tib_top, m_tib_subjoined, m_tib_vowel, m_tib_final_wylie, m_tib_final_class, m_tib_other;
	...
	private WylieTsekbar fromWylieOneTsekbar(String[] tokens, int i) {
		...
		if (this.check && warns.size() == 0 && check_root && root_idx >= 0) {
			...
			if (consonants.size() == 2 && root_idx != 0 && prefix(consonants.get(0), consonants.get(1)) && isSuffix(consonants.get(1))) {
			...
			} else if (consonants.size() == 3 && isPrefix(consonants.get(0)) && suff2("s", consonants.get(1)) && consonants.get(2).equals("s")) {
	...
	private ToWylieTsekbar toWylieOneTsekbar(String str, int len, int i) {
		...
		if (stacks.size() > 2 && stacks.get(last).single_cons != null && stacks.get(last - 1).single_cons != null && isSuffix(stacks.get(last - 1).single_cons)
	...
	private ToWylieStack toWylieOneStack(String str, int len, int i) {
		...
		if (st.stack.size() == 1 && !st.stack.get(0).equals("a") && !st.caret && st.vowels.isEmpty() && st.finals.isEmpty()) {
	...
	private String putStackTogether(ToWylieStack st) {
		...
		} else if (!st.prefix && !st.suffix && !st.suff2 && (st.cons_str.isEmpty() || st.cons_str.charAt(st.cons_str.length() - 1) != 'a')) {
			out.append("a");
		}
